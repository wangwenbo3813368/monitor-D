<template>
    <div style="padding:0px;overflow:hidden;font-size:0">
      <div v-if="1==2" class="host-view-block host-view-block-inline-left" 
        style="font-size:13px;height:50px;width:calc((100% - 24px)*3 / 3);display:inline-block">
        <!--告警信息表格 高度根据数据的多少显示-此场景正常情况下的高度是比较小的-->
      </div> 

      <div class="host-view-block host-view-block-inline-left " 
      ...
      <div class="host-view-block host-view-block-inline-right "
  ...
  <draggable :options="{animation:300}" v-model="list">
      <div v-for="(item,index) in list" :key="index" 
           :class="getHostViewBlockClass(index) " 
           :style="getHostViewBlockStyle(3,1,3,null,'rgb(15,30,50)')">
        <!--cpu 内存图-->
        <div v-for="indexa in 10" :key="indexa">概述信息{{item.name}}</div>
      </div> 
  </draggable>

                             

      <div class="host-view-block host-view-block-inline-left host-view-block-inline-bottom" 
           :style="getHostViewBlockStyle(2,1,2)">
        <!--cpu 内存图-->
        <div v-for="index in 10" :key="index">概述信息5</div>
      </div> 
      <div class="host-view-block host-view-block-inline-right host-view-block-inline-bottom"
           :style="getHostViewBlockStyle(2,1,2)">
        <div v-for="index in 10" :key="index">概述信息6</div>
      </div> 
...
export default {
     ...
     components:{ 'cpu-memory':CpuMemoey,'disk':Disk,'netport':NetPort,'service':Service,draggable},
     data(){
        return {
