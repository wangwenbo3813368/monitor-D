...
export default {
    ...
    methods: {
        ...
        getHostViewContentStyle(number,numerator,denominator,heightScale){
            var heightScale = heightScale||4;
            var width = ((this.$store.state.sys.clientWidth- this.$store.state.sys.asideWidth)*this.$store.state.sys.hostContentWidthRate
            -24-(number-1)*6)*numerator/denominator-2;
            width = Math.floor(width* 100) / 100;
            return {
                width:width+"px",
        ...
        getHostViewBlockClass(index){
            if(index%3==0){
                return "host-view-block host-view-block-inline-left"
            }else if(index%3==1){
                return "host-view-block host-view-block-inline-middle"
            }else{
                return "host-view-block host-view-block-inline-right"
            }
        }
    }
